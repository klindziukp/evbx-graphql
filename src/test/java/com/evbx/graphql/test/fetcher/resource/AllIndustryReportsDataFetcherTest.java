...
class AllIndustryReportsDataFetcherTest extends BaseTest {
    ...
    private static final String ALL_REPORTS_JSON_STRING
            = "{\"items\":["
    ...
    void allIndustryReportsDataFetcherTest() {
        __GIVEN();
        stubWireMockServer(resourceServiceConfig.getIndustryReportPath(), ALL_REPORTS_JSON_STRING);
        __WHEN();
        ItemData<IndustryReport> data = allIndustryReportsDataFetcher.get(dataFetchingEnvironment);
        __THEN();
        Assertions.assertNotNull(data);
        Assertions.assertEquals(data, JsonUtil.fromJson(ALL_REPORTS_JSON_STRING, new TypeReference<ItemData<IndustryReport>>() {}));
    }
}